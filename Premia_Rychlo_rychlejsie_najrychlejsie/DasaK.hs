{-
original:
*Main> length $ backtrack3 allShapes3
11520
(13.94 secs, 7,162,141,304 bytes)

navrh2:
*Main> length $ backtrack3 allShapes3
11520
(9.63 secs, 7,236,837,176 bytes)
-}
...
--posielam zoznamy moznosti jednotlivych dielikov
allShapes3 :: [[Shape]]
allShapes3 = [plzeleneW, plzeleneS, plzlteT, plmodreLL, plcerveneLL, plmodreL, plcerveneV]

--spustam backtracking s tym, ze v jednotlichych krokoch vyfiltrujem tie, ktore sa uz nedaju 
--pouzit/umiestnit (ich pozicia je obsadena)
backtrack3 :: [[Shape]] -> [[Shape]]
backtrack3 [] = [[]]
backtrack3 (sh:shapes) = [ (a:b) | a <- sh, b <- backtrack3 
                            --([[z| z<-x, ((intersect z a) == [])] | x <- shapes]), -- original
                            -- navrh1: ([ filter (\z -> null (intersect a z)) x | x <- shapes ]),
                            -- navrh2: 
                            ([ filter (null . (intersect a)) x | x <- shapes ]),
                            isOK (a:b) ]
...
*Main> length $ backtrack3 allShapes3
11520
(15.80 secs, 7,162,142,664 bytes)
-}
