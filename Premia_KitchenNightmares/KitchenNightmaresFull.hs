...
alergoFriendly =  filter (\listok -> 0 == bitoveAND (alergeny listok) && all (\m -> pocetAlergenov m < 5) listok ) tyzdenny
                    where
                    alergeny :: Listok -> [Int]
                    alergeny listok  = map (\((p,pa),(h,ha)) -> (pa .|. ha))  listok
                    --
                    bitoveAND :: [Int] -> Int
                    bitoveAND = foldl (.&.) (1+2+4+8+16+32+64)
