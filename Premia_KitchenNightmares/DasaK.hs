...
121080
(352.86 secs, 154,513,787,448 bytes)
-}
...
isOkMenu :: Menu -> Int
isOkMenu m = sum[a| a <- [1,2,4,8,16,32,64], 
        (((snd (fst m)) .&. a) .|. ((snd (snd m)) .&. a)) == 0]
        
-- listok - 5 dvojjedala - kazdy alergen nesmie byt aspon v jednom dvojjedle
isOk :: Listok -> Bool
isOk x 
        | (length [m | m <- x, pocetAlergenov m < 5]) < (length x) = False
        | otherwise = 127 == (foldl(\a -> \x -> a .|. (isOkMenu x)) (0) x)

alergoFriendly :: [Listok]
alergoFriendly = [x | x <- tyzdenny, isOk x]

{-
*Main> length alergoFriendly
121080
(797.64 secs, 154,513,783,552 bytes)
*Main>
-}
