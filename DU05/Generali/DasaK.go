...
type Agent struct {
	...
	children map[string][]Message // strom prijatych sprav
...
func (agent *Agent) postorder(path string) int {
	...
	messages := agent.children[path]
	for index, msg := range messages {
		//ak je to list -> pridam hodnotu
		//inak vypocitam postorder pre children a tu pridam
		if index == 0 { //leaf
			lst = append(lst, msg.input)
		} else {
			if msg.level == 0 {
				lst = append(lst, msg.input)
			} else {
				lst = append(lst, agent.postorder(getKey(msg.path)))
			}
		}
	}
	return majority(lst) //vraciam majority listu
...
func majority(values []int) int {
	// toto mate doprogramovat
	// v premenej values su hodnoty 0/1
	a := 0 //a -> pocet 0 v premenej values
	...
	if 2*a >= len(values) {
		return 0 //ak je 0 viac alebo rovnako ako 1 vratim 0
	}
	return 1 //0 je menej ako 1 -> vratim 1
	//return 999999 // toto sa asi zmeni...
}
