...
var (
	...
	N = 10
	M = 3 // pocet generalov, pocet klamarov, 3610 sprav
	//N = 7
	//M = 2 // pocet generalov, pocet klamarov, 157 sprav
...
func (agent *Agent) postorder(path string) int {
	//princip ak som list tak moj input sa berie ako value
	//ak som vyssie tak moja value je majority z values podomnou
	messages := agent.children[path]
	if len(messages) == 1 {
		return messages[0].input
	}
	akt := []int{}
	for index, msg := range messages {
		if index > 0 { // first message is the original message received by agent
			akt = append(akt, agent.postorder(getKey(msg.path)))
		} else {
			akt = append(akt, msg.input) //nejak mi nesedi s algoritmom, pouzivat povodnu hodnotu ale asi je dovod na to aby sme ju mali ¯\_(ツ)_/¯
		}
	}
	return majority(akt)
}
...
func majority(values []int) int {
	//jednoduchy princip ratam nuly a jednotky
	//1 vratim iba ak bolo jednotiek viac teda pri remize vratim 0
	nuly := 0
	jednotky := 0
